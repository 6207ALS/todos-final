...
app.post("/lists/:todoListId/complete_all",
  ...
  catchError(async (req, res, next) => {
    ...
    req.flash("success", "Marked all todos as complete!");
    res.redirect(`/lists/${todoListId}`);
  })
);
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let createdTodo = await res.locals.store.createTodo(+todoListId, todoTitle);
      if (!createdTodo) throw new Error("Not found.");

      req.flash("The todo has been created.");
      res.redirect(`/lists/${todoListId}`);
    }
  })
);
  ...
  catchError(async (req, res, next) => {
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        req.flash("error", "Name must be unique.");
        await rerenderEditList();
      } else {
      ...
      if (store.isUniqueConstraintViolation(error)) {
        req.flash("error", "The list title must be unique :c");
        rerenderEditList();
      } else {
        throw error;
      }
